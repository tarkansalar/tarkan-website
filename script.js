...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * ======================================
     * SECTION 3: BLUE OCEAN DIAGNOSTIC QUIZ
     * ======================================
     */
    ...
    // --- 3A. QUIZ DATA BANK ---
    // All 21 questions are defined here in a structured format.
    const questionBank = [
        // Part 1: Brand Positioning (9 questions)
        {
            q: "What industry are you in?",
            type: 'select',
            options: ["Fashion & Apparel", "Cosmetics & Beauty", "Furniture & Home Goods", "Sports Equipment & Gear", "Supplements & Wellness", "Beverages (Coffee, Drinks, etc.)", "Other"]
        },
        {
            q: "What's your current annual revenue?",
            type: 'select',
            options: ["$500K – $1M", "$1M – $3M", "$3M – $5M", "$5M – $8M", "$8M – $10M+"]
        },
        {
            q: "If you were sold out of your best-selling product for 4 weeks, what would your customers do?",
            type: 'select',
            options: ["Wait for us to restock – they only want our brand", "Buy from a competitor – they need the product type, not ours", "Not sure – we've never tested this"]
        },
        {
            q: "If you raised your prices by 20% tomorrow, what would happen?",
            type: 'select',
            options: ["Sales would stay strong – our customers value what we offer", "We'd lose most customers – they're price-sensitive", "Not sure – we're afraid to test it"]
        },
        {
            q: "If your brand disappeared tomorrow, would your customers miss you—or just find a replacement?",
            type: 'select',
            options: ["They'd genuinely miss us – we have a loyal community", "They'd replace us easily – we're interchangeable", "Honestly, I don't know"]
        },
        {
            q: "When you pitch your brand to a stranger or investor, do they 'get it' immediately?",
            type: 'select',
            options: ["Yes – they instantly understand our category and value", "Mostly – they get it after a short explanation", "Not really – we have to explain multiple times", "No – people are often confused about what we actually do"]
        },
        {
            q: "Where do most of your customers discover you?",
            type: 'select',
            options: ["Paid ads (Meta, Google, TikTok)", "Organic social media", "Word of mouth / referrals", "Retail / wholesale partners", "Amazon or other marketplaces", "We're not sure – it's all mixed"]
        },
        {
            q: "What's your biggest competitive threat right now?",
            type: 'select',
            options: ["Cheaper knockoffs flooding the market", "Bigger brands with more budget", "Customer indifference – they don't see why we're different", "Market saturation – everyone sells the same thing", "Rising ad costs – can't acquire customers profitably anymore"]
        },
        {
        ...
        // Part 2: AI Readiness (6 questions)
        {
            ...
            type: 'multi-select',
            options: ["1D (Basic automation): ChatGPT for emails, captions, basic tasks", "2D (Cost savings): Automating customer service, operations, workflows", "3D (Unfair advantage): Using AI for product innovation, customer insights, etc.", "Not using AI at all", "Using AI but not sure if it's actually helping"]
        },
        {
            q: "If you had an extra $100K right now, where would you invest it to grow fastest?",
            type: 'select',
            options: ["Marketing & customer acquisition", "Product development & innovation", "Branding & positioning (to stand out)", "Systems, automation & AI tools", "Hiring & team", "I'm not sure – I need clarity first"]
        },
        {
        ...
        },
        {
            ...
            type: 'select',
            options: ["Strong system – we have dashboards, a CFO, or solid tools", "Manual tracking – spreadsheets, but inconsistent", "Accountant once a year – we're reactive, not proactive", "Honestly, we're not clear on real profits"]
        },
        {
        ...
        // Part 3: Coaching Fit (6 questions)
        {
            ...
            type: 'select',
            options: ["Yes, and it was transformational", "Yes, but it didn't work out (they didn't understand my business)", "No, but I'm open to it", "No, I prefer to figure it out myself"]
        },
        {
            ...
            options: ["Years of hands-on experience (10+ years)", "Built and scaled physical products themselves", "Deep expertise in my specific industry", "Proven frameworks and systems (not just theory)", "Someone who's been in the trenches", "Chemistry and trust"]
        },
        {
        ...
        },
        {
            ...
            type: 'select',
            options: ["Extremely urgent – it's keeping me up at night", "Very urgent – I need help within 30 days", "Somewhat urgent – I'll focus on it this quarter", "Not urgent – just exploring options"]
        },
        {
    ...
    function renderQuestions() {
        questionBank.forEach((q, index) => {
            ...
            switch (q.type) {
                ...
                case 'multi-select':
                    optionsHtml = q.options.map(option =>
                        `<button class="quiz-option multi" data-value="${option}">${option}</button>`
                    ).join('');
                    break;
                case 'open':
            ...
            const navHtml = `
                <div class="quiz-nav">
                    ...
                    ${index === questionBank.length - 1
                        ? `<button class="btn btn-primary btn-submit">Submit Results</button>`
    ...
    function handleOptionClick(optionBtn) {
        ...
        if (question.type === 'multi-select') {
            ...
            const selectedOptions = Array.from(optionBtn.parentNode.querySelectorAll('.selected')).map(btn => btn.dataset.value);
            ...
            if (question.limit && selectedOptions.length > question.limit) {
                ...
                // TODO: Show a small error message
                return;
            }
    ...
    function calculateAndShowResults() {
        ...
        if (answers[5] && (answers[5].includes("Not really") || answers[5].includes("No"))) {
            resultText1El.textContent = "Your answers show a critical lack of brand clarity. Customers and investors are likely confused, which directly costs you sales and wastes ad spend.";
        } else {
            resultText1El.textContent = "Your clarity seems high, but we can leverage this by building systems to ensure that clarity translates to every part of your operation.";
        }
        
        // 2. Loyalty Gap
        if (answers[2] && (answers[2].includes("Buy from a competitor") || answers[4].includes("They'd replace us easily"))) {
            resultText2El.textContent = "You're likely trapped in a 'Red Ocean,' competing on price, not value. This makes you vulnerable to cheaper knockoffs and market saturation.";
        } else {
            resultText2El.textContent = "You have a strong, loyal brand. Now we must build the operational engine to serve that community profitably and at scale.";
        }

        // 3. AI Leverage Gap
        if (answers[9] && (answers[9].includes("Not using AI at all") || answers[9].includes("1D (Basic automation)"))) {
            resultText3El.textContent = "You are currently using AI for basic tasks, but missing the 3D 'unfair advantage' to systemize operations, automate workflows, and drive product innovation.";
        } else {
            resultText3El.textContent = "You're already leveraging AI. Our sprint will focus on integrating it into a '3D Unfair Advantage' that your competitors can't copy.";
        }
